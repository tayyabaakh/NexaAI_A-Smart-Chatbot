...
# This module handles the full document ingestion pipeline for BappyGPT / NexaAI:
# 1. Environment & SSL Certificate Configuration (for secure API connections).
# 2. File Parsing across multiple formats (PDF, DOCX, TXT, MD, PY, CSV).
...
# # Initialize the Google Gemini embedding model to convert text strings into numerical vector arrays
...
# # Read raw text content from uploaded files based on file extensions
# def read_file_text(file_path: str) -> str:
#     """
#     Extracts raw text content from various file formats (.pdf, .docx, .txt, .md, .py, .csv).
    
#     Args:
#         file_path (str): Relative or absolute path to the target file.
        
#     Returns:
#         str: Extracted raw text content.
        
#     Raises:
#         ValueError: If the file type extension is unsupported.
#     """
#     path = Path(file_path)
...
#     # Read plain-text formatted source files
...
# # Parse, chunk, embed, and store document content in ChromaDB scoped to a thread_id
...
#         thread_id (str): Unique conversation thread ID to scope document ownership.

#     Returns:
#         dict: Metadata summary containing filename and chunk count.
#     """
...
#     # Configure text splitter with overlapping windows to preserve contextual continuity between chunks
...
# # Perform semantic similarity search against ChromaDB scoped to the user's thread_id
...
#     Args:
#         query (str): The search phrase or question emitted by the LLM/user.
#         thread_id (str): Conversation session ID to enforce isolated document retrieval.
#         k (int): Number of top matching text chunks to return. Default is 4.

#     Returns:
#         str: Formatted context block containing source attributions and text excerpts.
#     """
#     # Query Chroma vector store with metadata filtering to prevent cross-thread data leaks
...
#     # Format retrieved document chunks with clear source labels for LLM context injection
